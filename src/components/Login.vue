  ...
  <form @submit.prevent class="sign-in-form" >
    <fieldset v-if="showSignIn && !validToken" >
      ...
      <label for="sign-in" class="hidden" >Submit</label>
        <button name="sign-in" id="sign-in" @click="signIn" >
          <font-awesome-icon icon="sign-in-alt" />
        </button>
      <label for="toggleSignUp" class="hidden">
        Switch to {{ switchToLabel }}
      </label>
        <button name="toggleSignUp" id="toggleSignup" @click="toggleSignIn">
          <font-awesome-icon icon="sync-alt" />
        </button>
    </fieldset>
    <fieldset>
      <label v-if="!validToken" for="show-login" class="hidden">
        Show Login
      </label>
        <button 
          v-if="!validToken" 
    ...
    <p v-if="validToken && username" class="welcome">
      Welcome, {{ username }}!
    </p>
  </form>
</template>
...
export default {
  ...
  methods: {
    ...
    signIn() {
      ...
      login(
        url(urlKey), {user: {username: this.username, password: this.password}}
      )
        .then(result => {
          if(result.error) {
